...
def load_model(itos_filename, classifier_filename):
    ...
    model = get_rnn_classifier(bptt, 20*70, num_classes, vs, emb_sz=em_sz, n_hid=nh, n_layers=nl, pad_token=1,
            layers=[em_sz*3, 50, num_classes], drops=[dps[4], 0.1],input_p=dps[0], weight_p=dps[1], embed_p=dps[2], hidden_p=dps[3])


    # load the trained classifier
    #model[0].load_state_dict(torch.load('/Users/christiaanleysen/Documents/Projects/tfhub_tl/output_temp/ft_enc.pth', map_location='cpu'))
    #print('>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>')
    model.load_state_dict(torch.load(classifier_filename, map_location='cpu'),strict=True)
...
predict_input('/Users/christiaanleysen/Documents/Projects/tfhub_tl/output/itos.pkl','/Users/christiaanleysen/Documents/Projects/tfhub_tl/output/model_christiaan2.pth',)